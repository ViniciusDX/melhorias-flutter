...
class _DriversScreenState extends State<DriversScreen> {
  ...
  Future<void> _reload() async {
    ...
    try {
      ...
      for (final it in items) {
        ...
        final order = API.UsersGetFavoriteDriversCall.favOrder(it);

        final name = _readString(it, ['name', 'fullName', 'displayName']);
        final email = _readString(it, ['email', 'mail']);
        final phone = _readString(it, ['phone', 'phoneNumber', 'mobile', 'cell']);
  ...
  void _normalizeOrders() {
    final favs = _drivers.where((d) => d.favorite).toList()
  ...
  List<_Driver> get _filtered {
    ...
    final out = base.toList()
      ..sort((a, b) {
        final aFav = a.favorite ? 0 : 1;
        final bFav = b.favorite ? 0 : 1;
        if (aFav != bFav) return aFav - bFav;
        if (a.favorite && b.favorite) {
          return (a.order ?? 1 << 30).compareTo(b.order ?? 1 << 30);
        }
  ...
  Future<void> _moveToTop(_Driver d) async {
    ...
    setState(() {
      for (final x in _drivers) {
        if (x.id != d.id && x.favorite && x.order != null) x.order = (x.order!) + 1;
      }
      d.order = 1;
      _normalizeOrders();
    });
  ...
  Future<void> _toggleFavorite(_Driver d, bool v) async {
    ...
    setState(() {
      d.favorite = v;
      if (v) {
        final maxOrder = _drivers
            .where((x) => x.favorite && x.order != null)
            .fold<int>(0, (m, e) => e.order! > m ? e.order! : m);
        d.order = maxOrder + 1;
      } else {
        d.order = null;
      }
...
class _DriverCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget photoBadge() {
      final label = (data.favorite && data.order != null) ? '#${data.order}' : '-';
      final bg =
      (data.favorite && data.order != null) ? const Color(0xFF1E88E5) : Colors.black87;
      return Container(
